...
class TransactionControllerTest {
        ...
        @BeforeEach
        void setUp() {
                // Rebuild MockMvc with the ContentCachingFilter applied
                // This ensures the filter runs before the RateLimitInterceptor
                mockMvc = MockMvcBuilders.webAppContextSetup(webApplicationContext)
                                .addFilters(contentCachingFilter)
                                .build();
        }
        ...
        void shouldCreateTransactionWhenRequestIsValid() throws Exception {
                ...
                TransactionRequestDTO request = new TransactionRequestDTO(100, "Starbucks", now, 1);
                TransactionResponseDTO response = new TransactionResponseDTO(1, 100, "Starbucks", now, 1, "Tenpista 1");
                ...
                mockMvc.perform(post("/transactions")
                                ...
                                .andExpect(jsonPath("$.tenpista_id", is(1)));
        ...
        void shouldReturn400WhenCreateTransactionRequestIsInvalid() throws Exception {
                // Arrange
                TransactionRequestDTO invalidRequest = new TransactionRequestDTO(-10, "", null, null);
        ...
        void shouldReturnAllTransactions() throws Exception {
                ...
                List<TransactionResponseDTO> transactions = List.of(
                                new TransactionResponseDTO(1, 100, "Starbucks", now, 1, "Tenpista 1"),
                                new TransactionResponseDTO(2, 200, "Amazon", now, 1, "Tenpista 1"));
        ...
        void shouldUpdateTransactionWhenDataIsValid() throws Exception {
                ...
                Integer transactionId = 1;
                LocalDateTime now = LocalDateTime.now();
                TransactionRequestDTO request = new TransactionRequestDTO(150, "Updated Shop", now, 1);
        ...
        void shouldReturn429WhenRateLimitIsExceeded() throws Exception {
                // Arrange
                LocalDateTime now = LocalDateTime.now();
                TransactionRequestDTO request = new TransactionRequestDTO(100, "Starbucks", now, 10);
                TransactionResponseDTO response = new TransactionResponseDTO(1, 100, "Starbucks", now, 10,
                                "Tenpista 1");
                ...
                mockMvc.perform(post("/transactions")
                                ...
                                .andExpect(jsonPath("$.detail",
                                                is("Too many requests - Rate limit is 3 per minute for tenpista 10")));
